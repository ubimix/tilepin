 ...
 */
Tilepin.ConfigAdapter.MapnikPostgresLayers = Tilepin.ConfigAdapter.extend({
    ...
    $getMapnikCacheKey : function(params) {
        ...
        if (!that._getCacheKey) {
            ...
            'getKey', 'getId' ];
            that._getCacheKey = that.project.getConfigHandlerMethod(names);
        }
    ...
    _loadSQL : function() {
        var that = this;
        return Tilepin.P().then(function() {
            var obj = that.project.getConfigObject();
            return Tilepin.P.all(_.map(obj.Layer, function(dataLayer) {
                return Tilepin.P().then(function() {
                    if (!dataLayer.Datasource)
                        return;
                    ...
                    if (file && !table) {
                        var projectDir = that.project.getProjectDir();
    ...
    _loadProjectStyle : function(stylesheet) {
        ...
        if (_.isString(stylesheet)) {
            ...
            promise = Tilepin.P().then(function() {
                return that.project._loadObject(fullPath);
            }).then(function(css) {
                if (css) {
                    return css;
                }
...
Tilepin.ConfigAdapter.MapnikAdapter = Tilepin.ConfigAdapter.extend({
    ...
    open : function() {
        ...
        return Tilepin.P().then(function(mml) {
            var confObj = that.project.getConfigObject();
            var projectDir = that.project.getProjectDir();
            var cacheDir = that.project.getCacheDir();
            return Tilepin.P.ninvoke(Millstone, 'resolve', {
                ...
                that.project.setConfigObject(obj);
                return obj;
            });
        })
    },
    ...
    getMapnikCacheKey : function(params) {
        var that = this;
        if (!that._getCacheKey) {
            var adapters = _.filter(that.project.getAdapters(), function(
                adapter) {
    ...
    prepareMapnikConfig : function(params) {
        ...
        return Tilepin.P().then(function() {
            var configObj = that.project.getConfigObject(true);
            var adapters = that.project.getAdapters();
...
Tilepin.ProjectConfig = Tilepin.Class.extend({
    ...
    initialize : function(options) {
        ...
        _.each([ Tilepin.ConfigAdapter.MapnikPostgresLayers,
    ...
    getAdapter : function(Type) {
        ...
        var adapter = that._adapterIndex[Type._typeId];
        return adapter;
    },
    ...
    _registerAdapter : function(Type) {
        var that = this;
        var adapter = that._adapterIndex[Type._typeId] = new Type({
            project : that
        });
        return adapter;
    },
    ...
    /** Loads project configuration from the file */
    _loadProjectConfig : function() {
        var that = this;
        var project = that.options.project;
        // There is already pre-loaded configuration. Just return it.
        if (project) {
            return Tilepin.P(project);
        }
